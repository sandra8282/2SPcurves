...
btspIPW <- function(res, maindat, B, level, xlab, ylab, main, cex.axis = cex.axis,
    ...
    conf.lev = (1-level)/2
     ...
     for (b in 1:B) {
       ...
       AKMests <- with(btspdat, getAKMtab(time, event, group, weights))
       temp <- completeROC(AKMests[[1]], silent = TRUE)
       bstAUC[b] <- temp$AUC
       forplot <- temp$R_u
