...
#' @param weights passed from ROCsurv.
...
PHMiptw <- function(time, event, group, weights, frmla, data, silent, abtwc, xlab, ylab, main, cex.axis,
                      cex.lab, lty, label.inset, label.cex, lwd){

  coxfit <- coxph(frmla, ties = "breslow")
  AKMres <- getAKMtab(time, event, group, weights)
  skm <- AKMres[[1]]
