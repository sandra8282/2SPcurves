...
comprsk_c <- function(mymat, rlabels, maxt){
  ...
  mymat=b[a,roll=+1]
  indsna <- which(is.na(mymat$w)&mymat$i.time<maxt)
  mymat$w[indsna] = 1;
  mymat <- mymat[,-c(1,3)]
  indsna <- which(is.na(mymat$w)&mymat$i.time>maxt)
  mymat$w[indsna] = 0;
  ...
  for (e in 1:max(mymat$event)){
    for (i in 1:n){
      for (j in 1:n){
        ...
        #     }
        if (mymat$time[i]<maxt){
          temp1 <- ifelse((mymat$group[i]<mymat$group[j]& mymat$event[i]==e)|
                            (mymat$group[i]>=mymat$group[j] & mymat$event[i] %!in% c(0,e)), 1, 0)
                  #
          temp2 <- ifelse(mymat$time[i]<mymat$time[j] & ((mymat$group[i]<mymat$group[j]& mymat$event[i]==e)|
                            (mymat$group[i]>=mymat$group[j] & mymat$event[i] %!in% c(0,e))), 1, 0)
          denom = denom + temp1; num = num + temp2;
          ####
          if (mymat$w[i]!=0 & mymat$w[j]!=0 & mymat$w2[i]!=0 & mymat$w2[j]!=0){
     ...
     c_p[e] <- num/denom; c_u[e] <- num2/denom2; #c_h[e] <- cij/Q;
    c_w1[e] <- num3/denom3; c_w2[e] <- num4/denom4; c_w3[e] <- num5/denom5; c_u2[e] = num6/denom6
  }
  #names(c_h) =
  names(c_p) = names(c_u) = names(c_u2) = names(c_w1) = names(c_w2) = names(c_w3) = rlabels

  return(list( c_p = c_p, c_u=c_u, c_u2 = c_u2, c_w1=c_w1, c_w2=c_w2, c_w3=c_w3)) #c_h=c_h,
}
