...
comprsk_c <- function(mymat, rlabels, maxt){
  ...
  for (e in 1:max(mymat$event)){
    for (i in 1:n){
      for (j in 1:n){
        if (i!=j){
          temp <- ifelse((mymat$event[j]==e & mymat$group[i]==1 & mymat$group[j]==0)|mymat$event[i] %!in% c(e,0), 1, 0)
          dnomcount = dnomcount + temp
          temp2 <- ifelse(mymat$i.time[j] <= mymat$i.time[i] & ((mymat$event[j]==e &
                                                                   mymat$group[i]==1 & mymat$group[j]==0)|mymat$event[i] %!in% c(e,0)), 1,0)
